...
"""Dynamic Flow Convolution Module"""
...
class DFConv(nn.Module):
    def __init__(
        ...
        """
        A Dynamic Flow Convolution Implementation
        ...
        self.gn = nn.GroupNorm(out_channels // 5, out_channels)
...
def get_coordinate_map_2D(
    ...
    """Computing 2D coordinate map  based on: TODO
    ...
    if orientation not in (0, 1):
        raise ValueError("orientation should be 0 or 1.")
    ...
    if morph == 0:
        ...
        if orientation == 0:
            for index in range(1, center + 1):
                absolute = torch.abs(y_offset_new_[center + index - 1])-torch.abs(y_offset_new_[center + index])
                sign1 = torch.sign(y_offset_new_[center + index - 1])
                result1 = torch.where(absolute>0, 1, 0)
                result2 = 1-result1
   

                # new_offset_y1 = result1* y_offset_new_[center + index - 1] +sign1*result2* y_offset_new_[center + index]
                new_offset_y1 = result1 * y_offset_new_[center + index - 1].clone() + sign1 * result2 * y_offset_new_[center + index].clone()

                y_offset_new_[center + index] = (
                    new_offset_y1 + y_offset_[center + index]
                )
                ...
                # new_offset_y2 = result2* y_offset_new_[center - index + 1]+sign1*result1* y_offset_new_[center - index]
                new_offset_y2 = result2 * y_offset_new_[center - index + 1].clone() + sign1 * result1 * y_offset_new_[center - index].clone()

                y_offset_new_[center - index] = (
                    new_offset_y2 + y_offset_[center - index]
                )
        elif orientation == 1:
            for index in range(1, center + 1):
                absolute = torch.abs(y_offset_new_[center + index - 1])-torch.abs(y_offset_new_[center + index])
                ...
                # new_offset_y1  = result2* y_offset_new_[center + index - 1]+sign1*result1* y_offset_new_[center + index]
                new_offset_y1  = result2 * y_offset_new_[center + index - 1].clone() + sign1 * result1 * y_offset_new_[center + index].clone()
                ...
                # new_offset_y2 = result1* y_offset_new_[center - index + 1] +sign1*result2* y_offset_new_[center - index]
                new_offset_y2 = result1 * y_offset_new_[center - index + 1].clone() + sign1 * result2 * y_offset_new_[center - index].clone()
        ...
        if orientation == 0:
            for index in range(1, center + 1):
                absolute = torch.abs(x_offset_new_[center + index - 1])-torch.abs(x_offset_new_[center + index])
                sign1 = torch.sign(x_offset_new_[center + index - 1])
                result1 = torch.where(absolute>0, 1, 0)
                result2 = 1-result1

                # new_offset_x1  = result1* x_offset_new_[center + index - 1] +sign1*result2* x_offset_new_[center + index]
                new_offset_x1 = result1 * x_offset_new_[center + index - 1].clone() + sign1 * result2 * x_offset_new_[center + index].clone()

                x_offset_new_[center + index] = (
                    new_offset_x1.clone() + x_offset_[center + index]
                ).clone()
                absolute = torch.abs(x_offset_new_[center - index + 1])-torch.abs(x_offset_new_[center - index])
                sign1 = torch.sign(x_offset_new_[center - index + 1])
                result1 = torch.where(absolute>0, 1, 0)
                ...
                # new_offset_x2  = result2* x_offset_new_[center - index + 1] +sign1*result1* x_offset_new_[center - index]
                new_offset_x2 = result2 * x_offset_new_[center - index + 1].clone() + sign1 * result1 * x_offset_new_[center - index].clone()


                x_offset_new_[center - index] = (
                    new_offset_x2+ x_offset_[center - index]
                )
        elif orientation ==1:
            for index in range(1, center + 1):
                absolute = torch.abs(x_offset_new_[center + index - 1])-torch.abs(x_offset_new_[center + index])
                ...
                # new_offset_x1  = result2* x_offset_new_[center + index - 1] +sign1* result1 *x_offset_new_[center + index]
                new_offset_x1 = result2 * x_offset_new_[center + index - 1].clone() + sign1 * result1 * x_offset_new_[center + index].clone()


                x_offset_new_[center + index] = (
                    new_offset_x1  + x_offset_[center + index]
                )
                ...
                # new_offset_x2 = result1 * x_offset_new_[center - index + 1] + sign1 * result2 * x_offset_new_[center - index]
                new_offset_x2 = result1 * x_offset_new_[center - index + 1].clone() + sign1 * result2 * x_offset_new_[center - index].clone()
...
def get_interpolated_feature(
    ...
    """From coordinate map interpolate feature based on: TODO
